...
<body class="text-gray-100 select-none antialiased min-h-screen flex justify-center bg-black">
  <main class="w-full max-w-md bg-app-bg min-h-screen flex flex-col relative border-x border-white/5 pb-20 shadow-2xl">
    ...
    <header class="p-4 border-b border-white/5 flex justify-between items-center sticky top-0 bg-app-bg/95 backdrop-blur z-20">
      <div>
        <h1 class="font-bold text-sm text-white tracking-wide">سامانه معاملاتی خودکار A1</h1>
    ...
    <div class="flex-1 overflow-y-auto hide-scroll p-3 space-y-3" id="main-content">
      ...
      <section id="tab-dashboard" class="tab-pane space-y-3">
        ...
        <div class="grid grid-cols-2 gap-2">
          ...
          <button id="btn-dash-kill-quick" class="glass-card p-2.5 flex items-center justify-center gap-2 bg-app-red/10 border-app-red/20 hover:bg-app-red/20 transition-colors">
            <svg class="w-4 h-4 text-app-red" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke--app-green/10 text-app-green px-1.5 py-0.5 rounded border border-app-green/20">+۲.۴٪ روزانه</span>
          </div>
          <div class="h-32 w0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1v-4z"/></svg>
        ...
        <div class="glass-card p-3 border-r-2 border-r-app-primary">
          ...
          <p id="ai-engine-desc" class="text-[10px] text-gray-300 mt-2 leading-relaxed">در حال دریافت و تحلیل هوشمند داده‌های بازار...</p>
        </div>
      </section>
      ...
      <section id="tab-strategy" class="tab-pane hidden space-y-3">
        <div class="glass-card p-3">
          ...
          <div id="strategy-history-list" class="space-y-2 text-[10px]">
            <div class="text-center py-4 text-app-muted">در حال بارگذاری لاگ سیگنال‌ها...</div>
          </div>
        </div>
      </section>

      <!-- TAB 3: Portfolio & Orders -->
      <section id="tab-portfolio" class="tab-pane hidden space-y-3">
        <div class="glass-card p-3">
          <div class="flex justify-between items-center mb-3">
            <h2 class="text-xs font-bold text-white">دفترچه سفارشات شبیه‌ساز (Order Book)</h2>
            <button onclick="DashboardManager.submitTestOrder()" class="text-[9px] bg-app-green/20 text-app-green border border-app-green/30 px-2 py-0.5 rounded hover:bg-app-green/30">+ ثبت سفارش تست</button>
          </div>
          <div id="portfolio-orders-list" class="space-y-2 text-[10px]">
            <div class="text-center py-4 text-app-muted">در حال بارگذاری لیست سفارشات...</div>
          </div>
        </div>
      </section>
      ...
      <section id="tab-settings" class="tab-pane hidden space-y-3">
        <div class="glass-card p-3 space-y-3">
          <h2 class="text-xs font-bold text-white border-b border-white/5 pb-2">پیکربندی سیستم A1</h2>
          <div class="space-y-1.5 text-[10px] text-gray-300">
            <div class="flex justify-between py-1 border-b border-white/5"><span>وضعیت سرویس:</span><span class="text-app-green font-medium">a1-agent.service (Active)</span></div>
            <div class="flex justify-between py-1 border-b border-white/5"><span>موتور هوش مصنوعی:</span><span class="text-white">v1.0-local-hybrid (محلی)</span></div>
            <div class="flex justify-between py-1 border-b border-white/5"><span>منبع داده بازار:</span><span class="text-white">TSETMC CDN Proxy</span></div>
            <div class="flex justify-between py-1"><span>حالت معاملات:</span><span class="text-yellow-400">PAPER_TRADING_SIMULATOR</span></div>
          </div>
        </div>
      </section>
  ...
  <script>
    const DashboardManager = {
      ...
      switchTab(tabId) {
        ...
        if (tabId === 'tab-portfolio') this.fetchOrdersHistory();
      },
      formatNumber(val) {
      ...
      async fetchData() {
        try {
          ...
          if (aiRes.ok) {
            ...
            if (aiData.signal && aiData.signal.includes('SELL')) {
              signalElem.className = 'font-bold text-[11px] text-app-red';
            } else if (aiData.signal && aiData.signal.includes('BUY')) {
            ...
            if (data.history && data.history.length > 0) {
              container.innerHTML = data.history.reverse().map(item => `
                <div class="bg-black/30 p-2.5 rounded border border-white/5 flex justify-between items-center">
                  <div>
                    <div class="font-bold text-white">${item.signal}</div>
                    <div class="text-[9px] text-app-muted">${item.time_iso || '--'}</div>
                  </div>
                  <div class="text-right">
                    <div class="text-app-orange font-medium">ریسک: ${(item.risk_score * 100).toFixed(0)}٪</div>
                    <div class="text-[9px] text-app-muted">${item.model}</div>
                  </div>
                </div>
              `).join('');
            } else {
              container.innerHTML = '<div class="text-center py-4 text-app-muted">هیچ سیگنالی هنوز ثبت نشده است.</div>';
            }
          }
        } catch (e) {
          container.innerHTML = '<div class="text-center py-4 text-app-red">خطا در دریافت لاگ استراتژی</div>';
        }
      },
      async fetchOrdersHistory() {
        const container = document.getElementById('portfolio-orders-list');
        try {
              ...
              container.innerHTML = data.orders.reverse().map(ord => `
                <div class="bg-black/30 p-2.5 rounded border border-white/5 flex justify-between items-center">
                  <div>
                    ...
                    <div class="text-[9px] text-app-muted">${ord.time_iso} | ${ord.order_id}</div>
                  </div>
                  <div class="text-right">
                    <div class="text-white font-medium">${new Intl.NumberFormat('fa-IR').format(ord.total_value)} ریال</div>
                    <div class="text-[9px] text-app-muted">${new Intl.NumberFormat('fa-IR').format(ord.volume)} سهم @ ${new Intl.NumberFormat('fa-IR').format(ord.price)}</div>
                  </div>
                </div>
              `).join('');
            } else {
              container.innerHTML = '<div class="text-center py-4 text-app-muted">هنوز سفارشی ثبت نشده است.</div>';
            }
          }
        } catch (e) {
          container.innerHTML = '<div class="text-center py-4 text-app-red">خطا در دریافت لیست سفارشات</div>';
        }
      },
      async submitTestOrder() {
        try {
          const res = await fetch('/api/trade/execute', {
            ...
            body: JSON.stringify({ symbol: 'فولاد', side: 'BUY', volume: 1000, price: 4900 })
          });
          if (res.ok) {
            this.fetchOrdersHistory();
          }
        } catch (e) {
          alert('خطا در ثبت سفارش تست');
        }
      },
      initChart() {
...
echo "Phase 23 Full Dynamic UI Deployed."
